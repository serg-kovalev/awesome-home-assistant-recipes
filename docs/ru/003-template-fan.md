# Шаблонный `fan` поверх одиннадцати булевых точек
...
![Сырые сущности LocalTuya: одиннадцать переключателей](../assets/screenshots/localtuya-raw-switches.jpg)

Одиннадцать тумблеров работают, но пользоваться ими неудобно, и главное — их не понимает ни голосовой
...
выходит по двум причинам.

**Режимы у прибора — отдельные булевы точки, а не перечисление.** Сущность `fan` и в tuya-local, и в
шаблонах ожидает одну точку со списком значений (`low` / `medium` / `high` в одном DP). Три независимых
...
`fan` умеет проценты, а документация Home Assistant прямо даёт раскладку для трёх скоростей: 33, 66,
100. Читаем текущее значение из трёх переключателей:
...
видно с задержкой одного опроса: секунду-две два переключателя могут выглядеть включёнными
одновременно.
...
## Три обхода прошивочных странностей
...
Плюс практическая выгода: без условия каждая смена режима на работающем приборе тормозила бы на две
...
Поэтому влажность вынесена в отдельный `select`, а не в три переключателя: у него есть значение `off`,
...
Плитка `fan`: питание, четыре кнопки скорости (выключено, low, medium, high) и выпадающий список
режимов. Ниже — `select` влажности и два датчика, под ними история за двенадцать часов. Пилообразный
график температуры — это работа рекуперации: прибор периодически переключает направление потока (те самые
два цикла рекуперации из инструкции), и датчик видит то воздух с улицы, то из помещения.

Одиннадцать сырых переключателей LocalTuya при этом никуда не делись — они остаются под капотом как
отвёртка для отладки, просто не вынесены на дашборд.
