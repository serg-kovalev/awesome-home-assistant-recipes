...
## 3 обхода прошивочных странностей
...
Плитка `fan`: питание, 4 кнопки скорости (выключено, low, medium, high) и выпадающий список
режимов. Ниже — `select` влажности и 2 датчика, под ними история за 12 часов. Пилообразный
график температуры — это работа рекуперации: прибор периодически переключает направление потока (те самые
2 цикла рекуперации из инструкции), и датчик видит то воздух с улицы, то из помещения.

11 сырых переключателей LocalTuya при этом никуда не делись — они остаются под капотом как
